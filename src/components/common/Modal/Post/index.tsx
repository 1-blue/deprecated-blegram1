    ...
    <StyledModal onClick={closePostModal}>
      <div>
        <button type="button" onClick={onClickBookmark}>
          <Icon shape="bookmark" size="sm" color="#000" hover="#FFF" />
          <span>{postModalData.isBookmarked ? "저장 취소" : "저장"}</span>
        </button>
        <button type="button" onClick={copyLink}>
          <Icon shape="link" size="sm" color="#000" hover="#FFF" />
          <span>링크</span>
        </button>
        {postModalData.isMine && (
          <>
            <button type="button" onClick={onClickUpdate}>
              <Icon shape="pencil" size="sm" color="#000" hover="#FFF" />
              <span>수정</span>
            </button>
            <button type="button" onClick={onClickDelete}>
              <Icon shape="trash" size="sm" color="#000" hover="#FFF" />
              <span>삭제</span>
            </button>
          </>
        )}
