...
import type { ApiFetchBookmarkedPostsResponse } from "@src/types/api";

/** 2023/05/26 - 특정 유저가 북마크한 게시글들 엔드포인트 - by 1-blue */
const handler: NextApiHandler<ApiFetchBookmarkedPostsResponse> = async (
  req,
  res
) => {
  try {
    // 특정 유저가 북마크한 게시글들 가져오기 요청
    if (req.method === "GET") {
      ...
      const posts = await prisma.post.findMany({
        where: { bookMarkers: { some: { bookmarkerIdx: targetUser.idx } } },
        take,
        skip: lastIdx === -1 ? 0 : 1,
    ...
    console.error("/api/posts/bookmarked error >> ", error);
