...
import type { ApiFetchLikedPostsResponse } from "@src/types/api";

/** 2023/05/26 - 특정 유저가 좋아요한 게시글들 엔드포인트 - by 1-blue */
const handler: NextApiHandler<ApiFetchLikedPostsResponse> = async (
  req,
  res
) => {
  try {
    ...
    if (req.method === "GET") {
      ...
      const posts = await prisma.post.findMany({
        where: { postLikers: { some: { postLikerIdx: targetUser.idx } } },
        take,
        skip: lastIdx === -1 ? 0 : 1,
    ...
    console.error("/api/posts/liked error >> ", error);
